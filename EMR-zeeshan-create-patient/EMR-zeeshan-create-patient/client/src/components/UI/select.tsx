...
>(({ className, children, position = "popper", ...props }, ref) => {
  // Prevent keyboard navigation when input is focused
  const contentRef = React.useRef<HTMLDivElement | null>(null);

  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // If any input inside the content is focused, prevent Radix Select navigation
      if (contentRef.current) {
        const active = document.activeElement;
        if (active && contentRef.current.contains(active) && active.tagName === "INPUT") {
          // Prevent navigation keys
          if (["ArrowDown", "ArrowUp", "Enter", "Home", "End", "PageDown", "PageUp"].includes(e.key)) {
            e.stopPropagation();
            e.preventDefault();
          }
        }
