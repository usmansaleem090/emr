...
export default function AddTaskPage() {
  ...
  const loadAssignableUsers = async () => {
    ...
    try {
      console.log("AddTaskPage - Loading assignable users for clinic:", user.clinicId);
      const response = await api.get(
        `/api/tasks/assignable-users/${user.clinicId}`
      );
      console.log("AddTaskPage - Assignable users response:", response.data);
      ...
      console.log("AddTaskPage - Assignable users extracted:", usersData);
      setAssignableUsers(usersData);
    } catch (error: any) {
      console.error("AddTaskPage - Failed to load assignable users:", error);
      toast({
        title: "Error",
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      console.log('AddTaskPage - Files selected via input:', selectedFiles);
      setFiles((prev) => {
        const newFiles = [...prev, ...selectedFiles];
        console.log('AddTaskPage - Updated files state:', newFiles);
        return newFiles;
      });
    }
  };
  ...
  const handleFileDrop = (e: React.DragEvent) => {
    ...
    console.log('AddTaskPage - Files dropped:', droppedFiles);
    setFiles((prev) => {
      const newFiles = [...prev, ...droppedFiles];
      console.log('AddTaskPage - Updated files state after drop:', newFiles);
      return newFiles;
    });
  };
  ...
  const removeFile = (index: number) => {
    console.log('AddTaskPage - Removing file at index:', index);
    setFiles((prev) => {
      const newFiles = prev.filter((_, i) => i !== index);
      console.log('AddTaskPage - Updated files state after removal:', newFiles);
      return newFiles;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('AddTaskPage - Files count:', files.length);
      console.log('AddTaskPage - Form data:', formData);

      if (files.length > 0) {
        // CASE 1: Files present - Use FormData for multipart upload
        console.log('AddTaskPage - Using FormData submission with files');
        ...
        files.forEach((file, index) => {
          console.log(`AddTaskPage - Adding file ${index + 1}:`, file.name, file.size, file.type);
          formDataToSend.append('files', file);
        });

        // Debug: Log FormData contents
        console.log('AddTaskPage - FormData entries:');
        ...
        console.log('AddTaskPage - FormData submission successful:', response.data);
        
      } else {
        // CASE 2: No files - Use JSON for faster processing
        console.log('AddTaskPage - Using JSON submission (no files)');
        ...
        console.log('AddTaskPage - JSON payload:', jsonPayload);

        const response = await api.post('/api/tasks', jsonPayload, {
          headers: { 'Content-Type': 'application/json' },
        });
        console.log('AddTaskPage - JSON submission successful:', response.data);
      }
      ...
      console.error("AddTaskPage - Failed to create task:", error);
      console.error("AddTaskPage - Error response:", error.response?.data);
      console.error("AddTaskPage - Error status:", error.response?.status);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Create a new task for your clinic team
          </p>
        </div>
      </div>
            ...
            <div className="space-y-2 w-full">
              ...
              >
                ...
                <input
                  ...
                  className="hidden"
                  id="file-upload-add-task"
                />
                <label
                  htmlFor="file-upload-add-task"
